...
u_operators = (GROUPBY, PROJECTION, SELECTION, RENAME)  # List of unary operators

# Associates operator with python method
op_functions = {
    GROUPBY: 'groupby', PRODUCT: 'product', DIFFERENCE: 'difference', UNION: 'union', INTERSECTION: 'intersection', DIVISION: 'division', JOIN: 'join',
...
class Node:
    ...
    def toPython(self) -> CallableString:
        ...
        directly called.'''
        print("topythonpre", self._toPython())
...
def parse(expr: str) -> CallableString:
    ...
    if "~" in expr:
        print("bwoooop")
        expr = expr.split("//")
        return expr[3]+".groupby(["+expr[1]+"], ("+expr[2].split("+")[0]+", "+expr[2].split("+")[1]+"))"
        # return CallableString("dates.product(rooms)")
    print("treeexp", tree(expr))
    result = tree(expr).toPython()
    print("bwoop2", result)
    return result
